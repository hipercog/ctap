...
for i = 1:numel(fnames)
    ...
    for c = 1:2
        ...
        ERPS{i, c * 2 - 1} = ctap_get_erp(stan, 'loc', loc);
        ERPS{i, c * 2} = ctap_get_erp(devi, 'loc', loc);
        if Arg.PLOT
            [~, fn, ~] = fileparts(strrep(fnames{i}, '_session_meas', ''));
            ttl = sprintf('ERP%s_%s_%s-tones', Arg.loc_label, fn, cond{c});
%             erp, src, pnts, zeropt, srate, tkoffset, lgnd, ttl, savename
            ctap_plot_basic_erp([ERPS{i, c * 2 - 1}; ERPS{i, c * 2}]...
                , stan.pnts, stan.pnts / 2, eeg.srate...
                , 'lgnd', {[cond{c} ' standard'] [cond{c} ' deviant']}...
                , 'ttl', ttl...
                , 'savename', fullfile(Cfg.env.paths.exportRoot, [ttl '.png']))
        end
