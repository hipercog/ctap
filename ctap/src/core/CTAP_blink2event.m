...
if isfield(EEG.CTAP, 'detected')
    if isfield(EEG.CTAP.detected, 'blink')
        ...
        x = randn(1, length(EEG.CTAP.detected.blink.QCData.criterionValue));
        plot(x(EEG.CTAP.detected.blink.QCData.isBlink),...
            EEG.CTAP.detected.blink.QCData.criterionValue(EEG.CTAP.detected.blink.QCData.isBlink),...
            'ro', ...
            x(~EEG.CTAP.detected.blink.QCData.isBlink),...
            EEG.CTAP.detected.blink.QCData.criterionValue(~EEG.CTAP.detected.blink.QCData.isBlink),...
            'ko');
        legend('blink','non-blink')
        ylabel(sprintf('Value of %s',...
            EEG.CTAP.detected.blink.QCData.criterionName));
        title('Blink detection criterion value (x-jittered)')
        set(gca, 'XTickLabel', '');
        
        
        savename = [EEG.CTAP.measurement.casename, '_',...
                    '_blink_criterion.png'];
        savefile = fullfile(get_savepath(Cfg, mfilename), savename);
