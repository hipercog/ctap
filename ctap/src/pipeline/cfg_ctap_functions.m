function Cfg = cfg_ctap_functions(Cfg, fun_args)
%CFG_CTAP_FUNCTIONS - Processes configuration file/struct with respect to the 
...
%
% Syntax:
%   Cfg = cfg_ctap_functions(Cfg, fun_args);
%
% Inputs:
...
% Define a color map for PSD.ting Quality Assurance figures
% Cfg.grfx.CTAP_CMAP = load_bwrc_colors(6);
...
if isfield(Cfg.env.paths, 'analysisRoot')
    ...
    % Log Files
    Cfg.env.logFile = fullfile(...
        Cfg.env.paths.logRoot, sprintf('runlog_%s.txt',datestr(now, 30)) );

else
   error('cfg_ctap_functions:cfgFieldMissing',...
         'Cfg.env.paths.analysisRoot is required for CTAP to work.'); 
end
...
if isfield(Cfg.eeg, 'reference')
    if ischar(Cfg.eeg.reference)
       Cfg.eeg.reference = cellstr(Cfg.eeg.reference);
...
%Cannot check contents of Cfg.eeg.reference here: channel names not available

if ~isfield(Cfg.eeg, 'chanlocs')
    warning('cfg_ctap_functions:cfgFieldMissing',...
...
if ~isfield(Cfg.pipe.stepSets, 'save') %check save instruction
    ...
    if numel(Cfg.pipe.stepSets) > 1 && ~Cfg.pipe.stepSets(1).save
        warning('cfg_ctap_functions:badSaveSpec',...
...
if ~fixcfg && ~ismember(Cfg.pipe.runSets, {Cfg.pipe.stepSets(1).id})
    %check for chanlocs
