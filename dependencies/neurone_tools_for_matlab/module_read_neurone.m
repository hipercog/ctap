...
%         headerOnly         : only read header information.
%         sessionPhaseNumber : specifies which session phase to be read,
%                              if multiple sessions are present within the
%                              same measurement. Defaults to
%                              sessionPhaseNumber = 1.
%         channels           : Cell array of strings with channel names
%                              to be read.
%
%  Output : A structure with all numerical data and all data from the
%           XML-files describing the recording setup.
%
...
%
%  Example: (A) recording = module_read_neurone('/NeurOne/data_01/')
%           
%           (B) channel_list = {'Fz','Cz', Pz'};
%               recording = module_read_neurone('/NeurOne/data_01/', 'channel', channel_list)
%
...
DATASETSESSIONFILE   = 'DataSetSession.xml';
...
if (str2double(recording.Session.TableInfo.Revision) > 1)
    error(strcat('This reader does not support the revision of Session.xml (', num2str(recording.Session.TableInfo.Revision), '). Please contact mega@megaemg.com for an update.'))
end
...
if (str2double(recording.Protocol.TableInfo.Revision) > 4)
    error(strcat('This reader does not support the revision of Protocol.xml (', num2str(recording.Protocol.TableInfo.Revision), '). Please contact mega@megaemg.com for an update.'))
end

% Read DataSetSession from XML-file, to get exam code etc
recording.DataSetSession = module_read_neurone_xml([basePath DATASETSESSIONFILE]);
...
if Arg.sessionPhaseNumber > nSessionPhases
    ...
    disp(['Reading session: ' num2str(Arg.sessionPhaseNumber)])
end
...
if ~(Arg.headerOnly)
    %% Determine which channels to read, based on input argument 'channels'
    ...
    if isempty(Arg.channels)
        ...
        [~, channel_index] = intersect({channelLayout.name}', Arg.channels);
        channel_index      = sort(channel_index);

        data = module_read_neurone_data(sessionDataFiles, channelCount, samplingRate, 'channels', channel_index, 'channelnames', Arg.channels);
    
        channelLayout = channelLayout(channel_index);
        channelCount  = numel(Arg.channels);
    end
    ...
    for n = 1:channelCount
        ...
        recording.signal.(channelName).data = calibratedMinimum + (data(:,n)-rawMinimum) / (rawMaximum-rawMinimum) * (calibratedMaximum-calibratedMinimum);
    ...
    % Store number of samples per channel
    [recording.properties.nSamplesPerChannel, ~] = size(data);
end
...
recording.properties.channellayout  = channelLayout;
...
recording.identifier = 'N/A';
    ...
    function [newData, channelLayout] = organiseData(table)
        ...
        [~, order]    = sort([channelLayout(:).inputNumber]);
